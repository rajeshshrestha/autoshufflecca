...
def split_image(data1, img_width,channel=3):
    data2 = deepcopy(data1)

    for subdata_idx, subdata in enumerate(data1):
        image_size = subdata[0].shape[-1]
        data1[subdata_idx] = (subdata[0].reshape(-1, channel, img_width, img_width)[:,:,:,:img_width//2].reshape(-1,image_size//2), subdata[1])

    for subdata_idx, subdata in enumerate(data2):
        image_size = subdata[0].shape[-1]
        data2[subdata_idx] = (subdata[0].reshape(-1, channel, img_width, img_width)[:,:,:,img_width//2:].reshape(-1,image_size//2), subdata[1])
    return data1, data2
